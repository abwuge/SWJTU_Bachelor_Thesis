...
The generated file is consumed by the official installer wrapper in
.github/scripts/setup_texlive.py, so the cache key follows source-level
dependency changes without maintaining a hand-written package list.
"""
...
def texlive_release(packages: dict[str, TexLivePackage]) -> str | None:
    config = packages.get("00texlive.config")
    if config is None:
        return None
    for dependency in config.depends:
        if dependency.startswith("release/"):
            return dependency.removeprefix("release/")
    return None
...
def write_package_file(
    ...
    lines = [
        ...
        f"# TeX Live release: {release or 'unknown'}",
